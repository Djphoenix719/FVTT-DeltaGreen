...
export class DGNPCSheet extends DGActorSheet<DGNPCSheetOptions, DGNPCSheetData, DGNPC> {
    ...
    /**
     * Called when an ability's damage is rolled.
     * @param event
     * @protected
     */
    protected async _handleRollAbilityDamage(event: JQuery.ClickEvent) {
    ...
    /**
     * Called when an ability's attack is rolled.
     * @param event
     * @protected
     */
    protected async _handleRollAbilityAttack(event: JQuery.ClickEvent) {
        const { id } = preprocessEventWithId(event);
        const ability = this.actor.items.get(id) as DGAbility;
    ...
    public activateListeners(html: JQuery) {
        ...
        html.find('label.clickable.roll.ability-damage').on('click', this._handleRollAbilityDamage.bind(this));
        html.find('label.clickable.roll.ability-attack').on('click', this._handleRollAbilityAttack.bind(this));
